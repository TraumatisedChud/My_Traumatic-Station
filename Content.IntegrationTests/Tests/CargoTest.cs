...
public sealed class CargoTest : GameTest
{
    ...
    public async Task NoStaticPriceAndStackPrice()
    {
        ...
        await server.WaitAssertion(() =>
        {
            // <Trauma> - optimise this shit by removing linq and caching component names. also don't have to check abstract it's always false
            var stackName = compFact.GetComponentName<StackComponent>();
            var priceName = compFact.GetComponentName<StackPriceComponent>();
            var staticName = compFact.GetComponentName<StaticPriceComponent>();
            foreach (var proto in protoManager.EnumeratePrototypes<EntityPrototype>())
            {
                if (pair.IsTestPrototype(proto) || !proto.TryGetComponent<StaticPriceComponent>(staticName, out var staticPriceComp))
                    continue;

                if (proto.TryGetComponent<StackPriceComponent>(priceName, out var stackPriceComp) && stackPriceComp.Price > 0)
                {
                ...
                if (proto.Components.ContainsKey(stackName))
                {
                    Assert.That(staticPriceComp.Price, Is.EqualTo(0),
