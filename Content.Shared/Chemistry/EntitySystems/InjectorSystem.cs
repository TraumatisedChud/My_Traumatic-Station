...
public sealed partial class InjectorSystem : EntitySystem
{
    ...
    private bool TryInject(Entity<InjectorComponent> injector, EntityUid user, EntityUid target, Entity<SolutionComponent> targetSolution, bool asRefill)
    {
        ...
        // <Trauma> - let the injector itself cancel it
        var usedEv = new InjectorBeforeInjectEvent(user, injector, target);
        RaiseLocalEvent(injector, ref usedEv);
        if (usedEv.Cancelled)
        {
            if (usedEv.OverrideMessage is {} msg)
        ...
        // <Trauma>
        var injectedEv = new AfterInjectedEvent(user, target);
        RaiseLocalEvent(injector, ref injectedEv);
        // </Trauma>
        ...
        FixedPoint2 transferAmount)
    {
        if (_solutionContainer.ResolveSolution(target.Owner, target.Comp.BloodSolutionName, ref target.Comp.BloodSolution))
        {
            ...
            UpdateFreshness(bloodTemp); // Trauma - On blood draw, freshness will almost always be at its best
    ...
    public void ToggleMode(Entity<InjectorComponent> injector, EntityUid user)
    {
        ...
        foreach (var allowedMode in injector.Comp.AllowedModes)
        {
            if (!ProtoMan.Resolve(allowedMode, out var proto)
                ...
                || GetSolution(injector) is not {} solution) // Trauma - use GetSolution
                continue;
